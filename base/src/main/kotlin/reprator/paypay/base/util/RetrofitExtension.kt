...
fun <T> Response<T>.toResultUnit(): PayPayResult<Unit> = try {
    if (isSuccessful) {
...
fun <T> Response<T>.toResult(): PayPayResult<T> = try {
    if (isSuccessful) {
...
suspend fun <T, E> Response<T>.toResult(mapper: suspend (T) -> E): PayPayResult<E> = try {
    if (isSuccessful) {
